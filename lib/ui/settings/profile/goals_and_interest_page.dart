    ...
    return UnfocusWidget(
      child: Scaffold(
        ...
        body: BaseView<ProfileViewModel>(
          ...
          onModelReady: (model) {
            // String defaultInterest = interests[0];
            model.preferredPartnersTraitController.text =
                user?.preferredAccountabilityPartnerTrait ?? "";
            // model.selectedInterest =
            //     "${user?.areasOfInterests[0] ?? defaultInterest}";
            model.longTermGoalController.text = user?.longTermGoal ?? "";
          },
